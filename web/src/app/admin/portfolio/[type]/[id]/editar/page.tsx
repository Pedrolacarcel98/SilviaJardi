import { getPortfolioItemById, PortfolioType, getPortfolioItems } from "@/data/mockPortfolio";
import PortfolioForm from "@/components/PortfolioForm";
import Link from "next/link";
import { notFound } from "next/navigation";

export function generateStaticParams() {
  const feriaItems = getPortfolioItems("feria").map((i) => ({ type: "feria", id: i.id }));
  const semanaSantaItems = getPortfolioItems("semana-santa").map((i) => ({ type: "semana-santa", id: i.id }));
  return [...feriaItems, ...semanaSantaItems];
}
...
export default async function PortfolioEditPage({
  ...
  const item = getPortfolioItemById(type as PortfolioType, id);
  if (!item) {
    notFound();
  }
    ...
    <main className="flex-grow w-full max-w-[1200px] mx-auto px-4 md:px-6 py-8 md:py-12 mt-4">
      ...
      <div className="text-center mb-10">
        ...
        <p className="font-body-md text-on-surface-variant mt-2 max-w-2xl mx-auto">
          Estás editando "{item.title}".
        </p>
      </div>

      <div className="max-w-4xl mx-auto">
        <PortfolioForm type={type as PortfolioType} initialData={item} />
      </div>
    </main>
  );
}
